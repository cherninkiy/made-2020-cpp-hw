...
bool check(const vector<ElemT>& a, const vector<ElemT>& b, double eps,
           double& alfa) {
...
ostream& operator<<(ostream& os, const vector<ElemT>& vec) {
  for_each(vec.begin(), vec.end(), [&os](ElemT x) { os << x << " "; });
  os << endl;
  return os;
}

/*
 * Оператор потокового ввода `>>`, принимающий первым числом размер, далее
 * значения
 */
template <typename ElemT>
...
template <typename ElemT>
vector<ElemT> operator%(const vector<ElemT>& a, const vector<ElemT>& b) {
  return vector<ElemT>{a[1] * b[2] - a[2] * b[1], a[2] * b[0] - a[0] * b[2],
                       a[0] * b[1] - a[1] * b[0]};
}
